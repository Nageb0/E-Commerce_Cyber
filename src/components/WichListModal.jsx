  ...
  return (
    ...
    >
      <motion.div
        ...
        className="relative w-full m-3 md:m-0 max-w-[720px] min-h-auto max-h-[80vh] bg-white rounded-3xl shadow-xl flex flex-col overflow-hidden"
      >
        <button
          aria-label="Close wishlist"
          onClick={clousModal}
          className="absolute top-4 right-4 text-black text-2xl p-1 rounded-md hover:bg-gray-100"
        >
          <IoClose className="text-2xl md:text-3xl" />
        </button>

        <h3 className="text-2xl md:text-3xl mt-5 md:mt-0 font-semibold text-black flex justify-center items-center p-3">
        ...
        <div className="p-4 overflow-y-auto flex-1 min-h-0">
          <div className="flex flex-col divide-y divide-gray-300">
            {products.length === 0 && (
              <p className="text-center font-bold text-2xl md:text-3xl text-black">
                No Products Added Yet !!
              </p>
            )}

            {products.map((el) => (
              <div key={el.documentId} className="flex items-center gap-4 py-4">
                <button onClick={() => {
                ...
                <div className="flex-1 min-w-0">
                  <h2 className=" text-[15px] md:text-lg font-semibold truncate">{el.name}</h2>
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <p className="text-[12px] md:text-xl font-semibold">{el.price} EGP</p>
                </div>
              </div>
            ))}
          </div>
